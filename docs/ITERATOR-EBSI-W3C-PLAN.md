...
Deliver one reviewed, locally committed development interoperability app that resolves
every explicitly registered EUDI and workspace W3C/OpenID profile, supports QR/deep-link
issuance and presentation, and permits valid development counterparties without trust
registry/status membership blocking the protocol.
...
8. Resolve inline credential offers and `credential_offer_uri`; support
    ...
    The local workspace compatibility set additionally includes issuer-state
    authorization challenge, final
    `urn:openid:dcp:ia:openid4vp_presentation`, draft bare
    `openid4vp_presentation`, final `ia_post`, draft `iar-post`, final
    `openid4vp_response.vp_token`, draft plain `vp_token`, both workspace credential
    proof shapes and DCQL matching. Draft 13/18 behavior is isolated by profile.
...
11. Implement diagnostic trust/status lookups where available. A valid but
    unregistered/unknown development issuer or verifier shows an informational
    one-shot Continue/Cancel warning and can continue. Invalid, expired, replayed,
    malformed or unsupported messages always reject.
...
9. Development warnings are one-shot and cannot override invalid, expired, replayed,
